...
export const Reportes = () => {
    ...
    useEffect(() => {
        const fecthInfo = async () => {
            try {
                const response = await fetch('${URL}/getDetalle');
                if (!response.ok) {
    ...
    useEffect(() => {
        ...
        const finalFilteredUsers = filteredUsers.filter(user => user.id_usuario !== 2);
        ...
        if (finalFilteredUsers.length > 0) {
            ...
            const dataValues = finalFilteredUsers.map(user => user.algunCampoDeDatos);
